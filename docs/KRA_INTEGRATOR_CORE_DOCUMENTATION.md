...
- `Core/Models/*` (see source tree for all request/response types)

If you need a shorter developer quick-start or example integration patch for a POS project (reference code-tested snippet), tell me which operation you will call (e.g. `SendSalesAsync`, `GetItemAsync`) and I will produce a minimal usage example and checklist.
