...
Public Class Stocks
    ...
    Private Async Sub BtnFetchStockMove_Click(sender As Object, e As EventArgs) Handles BtnFetchStockMove.Click
        Try
            ...
            LoadStockMoveGrid()
            CustomAlert.ShowAlert(Me, "Stock movement data fetched and saved successfully.", "Success",
    ...
    Private Async Sub BtnSaveStockMove_Click(sender As Object, e As EventArgs) Handles BtnSaveStockMove.Click
        Try
            ...
            Loader.Text = "Saving ..."
            ' Gather header info
            If DgvStockMoveHeader.Rows.Count = 0 OrElse DgvStockMoveHeader.Rows(0).IsNewRow Then
                Throw New Exception("Please enter header information before saving.")
            End If
            Dim headerRow As DataGridViewRow = DgvStockMoveHeader.Rows(0)
            Dim header As New StockMovementRecord With {
                .sarNo = headerRow.Cells("sar_no").Value?.ToString(),
                .orgSarNo = If(headerRow.Cells("org_sar_no").Value, headerRow.Cells("sar_no").Value?.ToString()),
                .regTyCd = headerRow.Cells("reg_ty_cd").Value?.ToString(),
                .custTin = headerRow.Cells("cust_tin").Value?.ToString(),
                .custNm = If(headerRow.Cells("cust_nm").Value, Nothing),
                .custBhfId = If(headerRow.Cells("cust_bhf_id").Value, Nothing),
                .sarTyCd = If(headerRow.Cells("sar_ty_cd").Value, Nothing),
                .ocrnDt = If(headerRow.Cells("ocrn_dt").Value, DateTime.Now.ToString("yyyyMMdd")),
                .totItemCnt = 0,
                .totTaxblAmt = 0D,
                .totTaxAmt = 0D,
                .totAmt = 0D,
                .remark = If(headerRow.Cells("remark").Value, Nothing),
            ...
            For Each row As DataGridViewRow In DgvStockMoveItems.Rows
                ...
                Dim item As New StockMovementItem With {
                    ...
                    .taxblAmt = Convert.ToDecimal(row.Cells("taxbl_amt").Value),
                    .taxTyCd = row.Cells("tax_ty_cd").Value.ToString(),
            ...
            Dim request As New StockMovementSaveRequest With {
                ...
                .sarNo = header.sarNo,
                .orgSarNo = header.orgSarNo,
                .regTyCd = header.regTyCd,
                .custTin = header.custTin,
                .custNm = Nothing,
                .custBhfId = header.custBhfId,
                .sarTyCd = header.sarTyCd,
            ...
            If res.resultCd = "000" Then
                CustomAlert.ShowAlert(Me, "Stock movement saved and uploaded successfully!", "Success",
                                        CustomAlert.AlertType.Success, CustomAlert.ButtonType.OK)
            Else
                CustomAlert.ShowAlert(Me, "Saved locally but upload failed: " & res.resultMsg, "Warning",
    ...
    Private Sub SetupHeaderGridColumns()
        ...
        DgvStockMoveHeader.Columns.Add("sar_no", "SARNo")
        DgvStockMoveHeader.Columns.Add("org_sar_no", "ORGSARNo")
        DgvStockMoveHeader.Columns.Add("reg_ty_cd", " RegTyCd")
        DgvStockMoveHeader.Columns.Add("cust_tin", "CustomerTIN")
        DgvStockMoveHeader.Columns.Add("cust_nm", "CustNm")
        DgvStockMoveHeader.Columns.Add("cust_bhf_id", "BranchID")
        DgvStockMoveHeader.Columns.Add("sar_ty_cd", "SarTyCd")
        DgvStockMoveHeader.Columns.Add("ocrn_dt", "OccurrenceDate")
        DgvStockMoveHeader.Columns.Add("tot_item_cnt", "TotalItems")
        DgvStockMoveHeader.Columns.Add("tot_taxbl_amt", "TaxableAmount")
        DgvStockMoveHeader.Columns.Add("tot_tax_amt", "TaxAmount")
        DgvStockMoveHeader.Columns.Add("tot_amt", "TotalAmount")
        DgvStockMoveHeader.Columns.Add("remark", "Remark")
    ...
    Private Sub SetupItemsGridColumns()
        ...
        DgvStockMoveItems.Columns.Add("sply_amt", "SupplyAmount")
        DgvStockMoveItems.Columns.Add("tot_dc_amt", "TotalDiscountAmount")
        DgvStockMoveItems.Columns.Add("taxbl_amt", "TaxableAmount")
        ' Tax type combo
        ...
        DgvStockMoveItems.Columns.Add("tax_amt", "TaxAmount")
        DgvStockMoveItems.Columns.Add("tot_amt", "TotalAmount")
        ' Grid settings
